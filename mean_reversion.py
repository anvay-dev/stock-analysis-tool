...
def run_reversion(training_data, testing_data=None):
    ...
    if testing_data is None:
        ...
        simulate_on["signal"] = np.where(
            (simulate_on["Close"] > simulate_on["lower_band"]) & (simulate_on["Close"].shift(1) < simulate_on["lower_band"].shift(1)), 1,
            np.where(
                (simulate_on["Close"] < simulate_on["upper_band"]) & (simulate_on["Close"].shift(1) > simulate_on["upper_band"].shift(1)), -1, 0))
    else:
        testing_data = testing_data.copy()
        testing_data.columns = testing_data.columns.get_level_values(0)
        simulate_on = testing_data
        simulate_on["signal"] = np.where(
            (simulate_on["Close"] > lower) & (simulate_on["Close"].shift(1) < lower), 1,
            np.where(
                (simulate_on["Close"] < upper) & (simulate_on["Close"].shift(1) > upper), -1, 0))
    ...
    last_close = float(simulate_on["Close"].dropna().iloc[-1])
    first_close = float(simulate_on["Close"].dropna().iloc[0])
    print(f"Close dtype: {simulate_on['Close'].dtype}")
    print(f"Last close: {simulate_on['Close'].iloc[-1]}")
    print(f"Type: {type(simulate_on['Close'].iloc[-1])}")
